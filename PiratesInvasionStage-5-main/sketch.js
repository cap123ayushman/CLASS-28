...
var cannon_water,pirare_laugh,cannon_explosion,background_music;
function preload() {
  ...
  pirare_laugh=loadSound("assets/pirare_laugh.mp3");
...
background_music.setVolume(0.002)
)
    ...
    for (var i = 0; i < boats.length; i++) {
      if (boats[i]) {
        ...
        if (collision.collided && !boats[i].isBroken) {
            ...
            if(!isLaughing && !pirare_laugh.isPlaying()){
              pirare_laugh.play();
              isLaughing = true
            }
          isGameOver = true;
...
function keyReleased() {
  if (keyCode === DOWN_ARROW) {
    ...
    cannon_explosion.setVolume(0.02)
